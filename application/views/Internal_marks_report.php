...
if($marks_type=="1"){
	$doc_title="<b>Termwork Marksheet";
	$max_marks='<label> Max Marks : 50</label>';
}
if($marks_type=="2"){
	$doc_title="<b>Practical Marksheet";
	$max_marks='<label> Max Marks : 50</label>';
}
if($marks_type=="3"){
	$doc_title="<b>Oral Marksheet";
	$max_marks='<label> Max Marks : 50</label>';
}
...
$pdf->SetTitle('INTERNAL MARKSHEET');
...
$table .='<tr>
			...
			<th style="border:1px solid black" align="center" ><b>Date</b><br></th>
			
		</tr>';
		
		$count_detain_student=0;
		foreach($internal_marks as $value){
			if($value->sem_internal_student_absent == '1'){
				$table .= '<tr>
				...
				<td style ="border:1px solid black" align="center">'.$value->date.'<br></td>
			    </tr>';

				}
			elseif($value->sem_internal_student_absent == '2') {
				$table .= '<tr>
							<td style ="border:1px solid black">  '.$value->student_enrollmentno.'<br></td>
			...
			elseif($value->sem_internal_marks_fail == '1') {
				$table .= '<tr>
							<td style ="border:1px solid black">  '.$value->student_enrollmentno.'<br></td>
							<td style ="border:1px solid black" align="center">'.$value->sem_internal_marks.' (FAIL)<br></td>
							<td style ="border:1px solid black" align="center">'.$value->date.'<br></td>
						   </tr>';
			}
			else{
				$table .= '<tr>
							<td style ="border:1px solid black">  '.$value->student_enrollmentno.'<br></td>
							<td style ="border:1px solid black" align="center">'.$value->sem_internal_marks.'<br></td>
							<td style ="border:1px solid black" align="center">'.$value->date.'<br></td>
						   </tr>';
			}
...
$table .='</table>';
$pdf->writeHTMLCell(0,0,'','',$type,0,1,0,true,'C',true);
...
foreach($No_Examiness_internal as $key => $data)
	$all_sum= $data->total;
foreach($absent_internal as $key => $data)
	$ab_sum= $data->total;
if($all_sum>$ab_sum){
...
ob_clean();
$pdf->Output('Internal Marksheet.pdf', 'I');
